...
o_user = products[grep("Organic",products$product_name),] # organic 상품을 주문한 유저들의 재구매 비유
...
users <- users %>% inner_join(us) %>%율 # 만든 4개의 features 붙이기. inner_join의 경우 object가 줄어들어 left_join으로
  left_join(o_user) %>%
