...
# Load Data ---------------------------------------------------------------
path <- "D:\\by0705\\Instacart\\data"

aisles <- fread(file.path(path, "aisles.csv"))
departments <- fread(file.path(path, "departments.csv"))
orderp <- fread(file.path(path, "order_products_prior.csv"))
ordert <- fread(file.path(path, "order_products_train.csv"))
...
o_id <- read.csv("o_id.csv")
theta <- read.table("lda\\theta20.txt", header = FALSE, sep = " ")
theta<- cbind(o_id, theta)
theta$o_index <- NULL

theta <- theta %>% group_by(user_id) %>% summarise(
	L1 = mean(V1),
	L2 = mean(V2),
	L3 = mean(V3),
	L4 = mean(V4),
...
p_id <- read.csv("p_id.csv")
phi <- read.table("lda\\phi20.txt", header = FALSE, sep = " ")
phi <- t(phi)
phi<- cbind(p_id, phi)
phi$p_index <- NULL
